...
public class IMGImage {
     ...
     * 涂鸦和马赛克路径集合
     */
    private List<IMGPath> mAllIMGPath = new ArrayList<>();
    ...
    public void undoAllIMGPath() {
        Log.e("aaa","mAllIMGPath.size()="+mAllIMGPath.size());
        if (!mAllIMGPath.isEmpty()) {
            IMGPath path = mAllIMGPath.get(mAllIMGPath.size() - 1);
            if (path.getMode() == IMGMode.DOODLE) {
                undoDoodle();
            } else if (path.getMode() == IMGMode.MOSAIC) {
                undoMosaic();
            }
            mAllIMGPath.remove(path);
        }
    }
