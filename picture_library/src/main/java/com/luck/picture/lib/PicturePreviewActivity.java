...
public class PicturePreviewActivity extends PictureBaseActivity implements
    ...
    private TextView tv_img_num, tv_title, picture_id_editor, tv_ok;
    private boolean isChooseOriginal = false;
    private RadioButton picture_rb_original;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        picture_rb_original = findViewById(R.id.picture_rb_original);
        tv_img_num = (TextView) findViewById(R.id.tv_img_num);
        tv_title = (TextView) findViewById(R.id.picture_title);
        position = getIntent().getIntExtra(PictureConfig.EXTRA_POSITION, 0);

        picture_id_editor = (TextView)findViewById(R.id.picture_id_editor);
        picture_id_editor.setVisibility(config.showEditor ? View.VISIBLE : View.GONE);
        picture_id_editor.setOnClickListener(this);
        ...
        isChooseOriginal = getIntent().getBooleanExtra(PictureConfig.CHOOSEORIGINAL, false);

        picture_rb_original.setVisibility(config.showOriginal ? View.VISIBLE : View.GONE);
        picture_rb_original.setChecked(isChooseOriginal);
        picture_rb_original.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (isChooseOriginal) {
                    isChooseOriginal = false;
                } else {
                    isChooseOriginal = true;
                }
                picture_rb_original.setChecked(isChooseOriginal);
                EventEntity obj = new EventEntity(PictureConfig.CHOOSEORIGINAL, isChooseOriginal);
        ...
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int i) {
                ...
                if(media.getPictureType().startsWith(PictureConfig.IMAGE)){
                    picture_id_editor.setVisibility(View.VISIBLE);
                }else{
                    picture_id_editor.setVisibility(View.GONE);
                }
    ...
    public void onSelectNumChange(boolean isRefresh) {
        ...
        if (enable) {
            picture_id_editor.setSelected(true);
            ...
            picture_id_editor.setSelected(false);
    ...
    public void onClick(View view) {
        ...
        //编辑
        if(id == R.id.picture_id_editor){
            editorImagePath = images.get(position).getPath();
//            Log.e("aaa","getPath="+editorImagePath);
//            // 涂鸦参数
//            DoodleParams params = new DoodleParams();
//            params.mIsFullScreen = false;
//            // 图片路径
//            params.mImagePath = editorImagePath;
//            // 初始画笔大小
...
//            // 启动涂鸦页面
//            DoodleActivity.startActivityForResult(PicturePreviewActivity.this, params, PictureConfig.DOODLE_EDITOR);

            Intent intent = new Intent(this, IMGEditActivity.class);
            Uri uri = Uri.fromFile(new File(editorImagePath));
            intent.putExtra(IMGEditActivity.EXTRA_IMAGE_URI, uri);
            startActivityForResult(intent , PictureConfig.DOODLE_EDITOR);
        }
        if (id == R.id.id_ll_ok) {
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                ...
                case UCrop.REQUEST_CROP:
                    ...
                    finish();
                    break;
                case PictureConfig.DOODLE_EDITOR:
                    String path = data.getStringExtra(IMGEditActivity.EXTRA_IMAGE_SAVE_PATH);
                    if (TextUtils.isEmpty(path)) {
                        return;
                    }
                    for (int i=0;i<images.size();i++) {
                        if (images.get(i).getPath().equals(editorImagePath)) {
                            images.get(i).setPath(path);
                            break;
                        }
                    }
                    for (int i=0;i<selectImages.size();i++) {
                        if (selectImages.get(i).getPath().equals(editorImagePath)) {
                            selectImages.get(i).setPath(path);
                            break;
                        }
                    }
                    adapter.notifyDataSetChanged();
                    EventEntity obj = new EventEntity(PictureConfig.DOODLE_REPLACE_PATH, editorImagePath, path , selectImages);
                    RxBus.getDefault().post(obj);
                    break;
            }
